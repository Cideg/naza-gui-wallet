...
//const char VERSION_DATA_URL[] = ""; // use ?1 trick to force reload and bypass cache
...
{
    setApplicationName("naza"); // do not change becasuse it also changes data directory under Mac and Win
    setApplicationDisplayName(tr("Naza Wallet"));
...
bool WalletApplication::init()
{
    ...
    QString path = dataDir.absoluteFilePath("naza-gui.lock");
    m_lockFile.reset(new QLockFile(path));

    if (!m_lockFile->tryLock())
    {
        WalletLogger::warning(tr("[Application] Naza wallet GUI already running"));
        QMessageBox::warning(nullptr, QObject::tr("Error"), tr("Naza wallet GUI already running"));
        return false;
    }
...
void WalletApplication::initSystemTrayIcon()
{
    ...
    m_systemTrayIcon->setIcon(QIcon(":images/naza_lin"));
...
{
    splashMsg(tr("Looks like this is your first run of the new Naza Wallet GUI. It is in detached state now.\nYou can open a wallet file, create a new one, or connect to remote wallet-rpc daemon."));
}
