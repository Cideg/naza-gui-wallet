...
//const char COMMUNITY_FORUM_URL[] = "http://t.me/nazacoin";
const char COMMUNITY_FORUM_URL[] = "https://naza.io";
const char REPORT_ISSUE_URL[] = "http://naza.io";

const char BUTTON_STYLE_SHEET[] =
        "QPushButton {border: none;}"
        "QPushButton:checked {background-color: #FF8C00; color: #FFFFFF}";
}
    ...
    , walletModel_(walletModel)
{
    ...
    setWindowIcon(QIcon(":images/naza_lin"));
    clearTitle();
...
//    qDebug("raw tx = %s", qPrintable(tx.binary_transaction));
    ...
    for (const RpcApi::Transfer& tf: tx.transaction.transfers)
    {
        ...
        msg.append(tr("%1 NAZA to %2\n").arg(amountStr).arg(addressStr));
    }
    msg.append(tr("Fee: %1 NAZA\n").arg(formatAmount(tx.transaction.fee)));
    msg.append(tr("Total send: %1 NAZA").arg(formatAmount(-ourAmount)));
...
void MainWindow::emailNaza() {
  EmailSendDialog emaildlg(this);
  emaildlg.exec();
}
...
//    m_ui->m_updateLabel->setText(QString("New version %1 of Naza wallet is available.").arg(QString("<a href=\"https://naza.io/downloads\">%1</a>").arg(newVersion)));
