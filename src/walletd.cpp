...
QString BuiltinWalletd::errorMessage(ReturnCode err)
{
    ...
    switch(err)
    {
    case ReturnCode::NAZAD_DATABASE_ERROR:
        msg = tr("Database write error. Disk is full or database is corrupted.");
        break;
    case ReturnCode::NAZAD_ALREADY_RUNNING:
        msg = tr("Cannot run nazad. Another instance of nazad is running.");
        break;
    case ReturnCode::WALLETD_BIND_PORT_IN_USE:
        msg = tr("Cannot run wallet-rpc. Wallet-rpc bind port in use.");
        break;
    case ReturnCode::NAZAD_BIND_PORT_IN_USE:
        msg = tr("Cannot run nazad. nazad bind port in use.");
        break;
    ...
    case ReturnCode::WALLET_FILE_EXISTS:
        msg = tr("The specified wallet file already exists. Naza wallet could not overwrite an existed file for safety reason. If you want to overwrite the file please remove it manually and try again.");
        break;
